...
function filterStaffData(list = [], searchVal = '') {
  ...
  return list.filter(row => {
    ...
    const staffId = String(row.staff_id || row.staffId || row.id || '').toLowerCase();
    const position = String(row.position || '').toLowerCase();
    const phone = String(row.phone_no || row.phoneNo || '').toLowerCase();

    return name.includes(q) || staffId.includes(q) || position.includes(q) || phone.includes(q);
  });
}
...
function renderStaffTable(rawData) {
  ...
  const filteredData = filterStaffData(rawData, gStaffSearch);
  const totalEntries = filteredData.length;

  renderStaffPagination(totalEntries);
  ...
  const endIndex = Math.min(startIndex + gStaffLimit, totalEntries);
  ...
  if (gStaffCategory === 'Full Time') {
    tbody.innerHTML = pageItems.map((item, idx) => {
      ...
      const basicAmt = Number(item.basic_amt ?? item.basicAmt ?? 0);
      const extraAmt = Number(item.extra_amt ?? item.extraAmt ?? 0);
      ...
      <tr class="hover:bg-slate-800/40 transition">
        ...
        <td class="text-right font-bold text-slate-200 py-3">${totalSalary.toLocaleString('en-US')}</td>
        <td class="text-right font-bold text-emerald-400 py-3">${bonus.toLocaleString('en-US')}</td>
...
function renderStaffPagination(totalRows) {
  ...
  const start = totalRows === 0 ? 0 : (gStaffPage - 1) * gStaffLimit + 1;
  const end = Math.min(gStaffPage * gStaffLimit, totalRows);

  if (info) info.innerHTML = `Showing <span class="text-indigo-400 font-extrabold">${start}</span> to <span class="text-indigo-400 font-extrabold">${end}</span> of <span class="text-indigo-400 font-extrabold">${totalRows}</span> entries`;

  if (btnPrev) btnPrev.disabled = (gStaffPage <= 1);
  if (btnNext) btnNext.disabled = (end >= totalRows);
}

function changePageStaff(delta) {
  const filteredData = filterStaffData(gStaffData, gStaffSearch);
  const totalPages = Math.ceil(filteredData.length / gStaffLimit) || 1;
  const newPage = gStaffPage + delta;
...
function renderGradeDropdownOptions(selectedValue = 'Non') {
  ...
  Object.keys(grades).forEach(g => {
    ...
    html += `<option value="${keyName}">${keyName} (${amt.toLocaleString('en-US')} MMK)</option>`;
  });
...
function calculateLiveStaffSalary() {
  ...
  const daysVal = parseFloat(document.getElementById('staff-working-days')?.value);
  const days = isNaN(daysVal) ? 26 : Math.max(0, daysVal);
  const isResigned = Boolean(document.getElementById('staff-resigned')?.value?.trim());
  ...
  if (pSalary) pSalary.textContent = `${totalSalary.toLocaleString('en-US')} MMK`;
  if (pBonus) pBonus.textContent = `${bonus.toLocaleString('en-US')} MMK`;
  if (pFund) pFund.textContent = `${fund.toLocaleString('en-US')} MMK`;
...
async function saveStaffForm(event) {
  ...
  if (!staffName) {
    ...
    if (typeof showToast === 'function') showToast("ERROR", "ဝန်ထမ်းအမည် ဖြည့်သွင်းရန် လိုအပ်ပါသည်။");
    return;
  }
  ...
  const resignedDate = document.getElementById('staff-resigned')?.value?.trim() || '';
  const isResigned = Boolean(resignedDate);
  ...
  const payload = {
    ...
    salaryGrade: (gStaffCategory === 'Full Time') ? (document.getElementById('staff-grade')?.value || 'Non') : 'Non',
    workingDays: (gStaffCategory === 'Full Time') ? days : 0,
    basicAmt: (gStaffCategory === 'Full Time') ? basic : 0,
    extraAmt: (gStaffCategory === 'Full Time') ? extra : 0,
    totalSalary: totalSalary,
    bonus: bonus,
    fund: fund,
    totalNetAmt: totalNetAmt,
    nrcNo: document.getElementById('staff-nrc')?.value?.trim() || '',
    bankAccount: document.getElementById('staff-bank')?.value?.trim() || '',
    phoneNo: document.getElementById('staff-phone')?.value?.trim() || '',
...
function exportToCSVStaff() {
  ...
  if (isFT) {
    ...
    gStaffData.forEach((r, idx) => {
      const displayNo = parseInt(r.no, 10) || (idx + 1);
...
window.fetchPayrollSettings = fetchPayrollSettings;
